...
main_func(double* res, char* key)
{
	...
	char	streamFile[200];/* STREAM FILENAME     */
...
//	generatorOptions(&streamFile);
	option = 0;
	sprintf(streamFile, "../data/binary_data/%s.bin", tp.key);
//	chooseTests();
...
{ 
	...
	sprintf(resultsDir, "../data/stats_output/%s/%s/%s/results.txt", tp.key, generatorDir[option], testNames[testNameID]);
	...
	for ( i=0; i<numOfFiles; i++ ) {
		if ( i < 10 )
			sprintf(s[i], "../data/stats_output/%s/%s/%s/data%1d.txt", tp.key, generatorDir[option], testNames[testNameID], i+1);
		else if (i < 100)
			sprintf(s[i], "../data/stats_output/%s/%s/%s/data%2d.txt", tp.key, generatorDir[option], testNames[testNameID], i+1);
		else
			sprintf(s[i], "../data/stats_output/%s/%s/%s/data%3d.txt", tp.key, generatorDir[option], testNames[testNameID], i+1);
	}
	numread = 0;
	m = numOfFiles/20;
	if ( (numOfFiles%20) != 0 )
		...
		if ( testVector[i] ) {
			...
			if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
				...
				for ( k=0; k<numOfFiles; k++ ) {
					if ( k < 10 )
						sprintf(s, "../data/stats_output/%s/%s/%s/data%1d.txt", tp.key, generatorDir[option], testNames[i], k+1);
					else if ( k < 100 )
						sprintf(s, "../data/stats_output/%s/%s/%s/data%2d.txt", tp.key, generatorDir[option], testNames[i], k+1);
					else
						sprintf(s, "../data/stats_output/%s/%s/%s/data%3d.txt", tp.key, generatorDir[option], testNames[i], k+1);
				...
				sprintf(s, "../data/stats_output/%s/%s/%s/results.txt", tp.key, generatorDir[option], testNames[i]);
				generalSampleSize = computeMetrics(s,i);
			}
		}
	}
