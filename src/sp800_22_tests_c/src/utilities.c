...
readBinaryDigitsInASCIIFormat(FILE *fp, char *streamFile)
{
	...
	printf("     Statistical Testing In Progress for %s.bin.........\n\n", tp.key);   
...
openOutputStreams(int option)
{
	...
	sprintf(freqfn, "../data/stats_output/%s/%s/freq.txt", tp.key, generatorDir[option]);
	if ( (freqfp = fopen_mkdir(freqfn, "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open freq file: <%s>", freqfn);
		exit(-1);
	}
	sprintf(summaryfn, "../data/stats_output/%s/%s/finalAnalysisReport.txt", tp.key, generatorDir[option]);
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if ( testVector[i] == 1 ) {
			sprintf(statsDir, "../data/stats_output/%s/%s/%s/stats.txt", tp.key, generatorDir[option], testNames[i]);
			sprintf(resultsDir, "../data/stats_output/%s/%s/%s/results.txt", tp.key, generatorDir[option], testNames[i]);
...
invokeTestSuite(int option, char *streamFile)
{
	...
	printf("     Statistical Testing for %s.bin Complete!!!!!!!!!!!!\n\n", tp.key);
}
